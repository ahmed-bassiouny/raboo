...
public class BluetoothLeService extends Service {
    ...
    private static BluetoothLeService _instance;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                SharedPreferencesUtil.saveString(BluetoothLeService.this, Const.CHANGEED_DEVICE_NAME, mBluetoothDeviceName);
                setHMNotify(gatt);
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (characteristic.getUuid().toString().equalsIgnoreCase(SampleGattAttributes.HM_RN_TX)) {
                    ...
                    L.i(TAG,"接收数据,read=" + Arrays.toString(ConvertHelper.byte2HexToStrArr(characteristic.getValue())));
                }
            }
        }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            L.i(TAG,"onCharacteristicChanged 返回=" + Arrays.toString(characteristic.getValue()));
        }
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Logger.t(TAG).i("写入监听成功:" + descriptor.getCharacteristic().toString());
            }
        }
     ...
     * @param gatt
     */
    private void setHMNotify(BluetoothGatt gatt) {
        Logger.t(TAG).i("先设置电量监听");
        ...
        for (BluetoothGattService bluetoothGattService : gattServiceList) {
            if (bluetoothGattService.getUuid().equals(UUID_HM_SERVICE)) {
                gattService = bluetoothGattService;
                break;
            }
        }
        BluetoothGattCharacteristic characteristicRXBattery = gattService.getCharacteristic(UUID_HM_RX_TX);//ffel 读写用的都是一个uuid
        setCharacteristicNotification(characteristicRXBattery, true);
    }
    ...
    private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
        ...
        if (data != null && data.length > 0) {
            ...
            Logger.t(TAG).i("broadcastUpdate 返回=" + String.format("%s", new String(data)));
    ...
    public static BluetoothLeService get_instance() {
        return _instance;
    }
    ...
    public boolean connect(final String address) {
        if (mBluetoothAdapter == null || address == null) {
            ...
            Logger.t(TAG).w("BluetoothAdapter not initialized or unspecified address.");
            return false;
        }
        ...
        if (mBluetoothDeviceAddress != null && address.equals(mBluetoothDeviceAddress)
                && mBluetoothGatt != null) {
            Logger.t(TAG).w("Trying to use an existing mBluetoothGatt for connection.");
        ...
        // parameter to false.
        mBluetoothGatt = device.connectGatt(this, false, mGattCallback);
        Logger.t(TAG).w("Trying to create a new connection.");
        mBluetoothDeviceAddress = address;
    ...
    public void writeCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        if (characteristic.getUuid().toString().equalsIgnoreCase(SampleGattAttributes.HM_RN_TX) && success) {
            LocalBroadcastManager.getInstance(BluetoothLeService.this).sendBroadcast(new Intent(Const.CHANGEED_DEVICE_NAME_BROADCASTER));
            Logger.t(TAG).i("写入名字成功，writeCharacteristic=" + Arrays.toString(ConvertHelper.byte2HexToStrArr(characteristic.getValue())));
        }
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
        ...
        if (UUID_HM_RX_TX.equals(characteristic.getUuid()) || UUID_HM_RN_TX.equals(characteristic.getUuid())) {
            try {
